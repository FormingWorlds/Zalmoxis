...
A failed solve is an error: when the solve at the Brent root is not finite, or the pressure solve finds no root and its last evaluation is not finite, `main` raises `StructureSolveError` with the outer and inner iteration, the radius and the stop, for both the Picard and the Newton outer solver.
