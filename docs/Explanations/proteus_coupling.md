...
4. Call `zalmoxis_solver(config, outdir, hf_row, num_spider_nodes=..., temperature_function=..., temperature_arrays=(r_arr, T_arr))`. The wrapper passes the interpolating callable as `temperature_function`, or `None` on JAX-viable layouts with `use_jax = true`. With `use_jax = true`, Zalmoxis takes T from the arrays everywhere and ignores `temperature_function`; with `use_jax = false`, Zalmoxis ignores the arrays and uses `temperature_function` (see [Outer solver and JAX path](#outer-solver-and-jax-path)).
