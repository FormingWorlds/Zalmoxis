...
4. Call `zalmoxis_solver(config, outdir, hf_row, num_spider_nodes=..., temperature_function=temperature_function, temperature_arrays=(r_arr, T_arr))`, with `temperature_function=None` on JAX-viable EOS layouts when `use_jax = true`. With `use_jax = true`, Zalmoxis takes T from the arrays everywhere and ignores `temperature_function`; with `use_jax = false`, Zalmoxis ignores the arrays and uses `temperature_function` (see [Outer solver and JAX path](#outer-solver-and-jax-path)).
...
2. `PALEOS-2phase:<solid>` (for example `PALEOS-2phase:MgSiO3`): separate solid + liquid tables. The liquidus is the PALEOS liquidus and the solidus is `mushy_zone_factor * liquidus`, the same curve pair that the structure solve receives for the two-phase density and the mushy-zone adiabatic gradient. Where the written phase boundaries follow the tables, the gap between the solid-table top and the liquid-table bottom sets the latent heat; where they are held at their running maximum (see the plateau below), the held values set it.

Each phase table is filled by inversion only inside the temperature window of that phase: the solid table up to the solidus and the melt table from the liquidus. Cells outside the window, such as melt-grid entropies below the liquidus or solid-grid entropies above the solidus (the solid entropy axis extends up to the melt range), and cells where PALEOS gives a non-finite value for any other property, are filled from the nearest valid cell in the same pressure column, because SPIDER cannot read NaN. `valid_mask_solid.dat` and `valid_mask_melt.dat` mark each cell 1 where it lies inside the temperature window and PALEOS gives a finite value for all five properties, and 0 where a value was filled. A cell whose temperature inverts but whose table adiabatic gradient is not finite keeps its other values, has the adiabatic gradient written as 0, and is 0 in the mask. Each mask is a plain-text grid of the integers 0 and 1 that `np.loadtxt` reads, with one row per entropy node and one column per pressure node, on the nodes of the property tables of the same phase.

**Liquidus entropy plateau.** The written solidus and liquidus entropies come from a PCHIP fit through 20 anchors spread over the pressure range of the table, and each curve is then held at its running maximum in pressure, so it is non-decreasing and flat above its highest value. For MgSiO3 the liquid-table entropy along the liquidus peaks near 164 GPa and then falls while the liquidus temperature keeps rising, since $\mathrm{d}S_\mathrm{liq}/\mathrm{d}P = -\alpha/\rho + (c_p/T)\,\mathrm{d}T_\mathrm{liq}/\mathrm{d}P$. With the falling curve, the first solid in a cooling molten mantle forms in a thin layer at the peak above still molten material, and the time step of coupled SPIDER and Aragog runs of 3 and 5 Earth-mass planets collapses at that onset. The plateau is therefore a solver-stability choice, not a property of the tables: the whole range from the peak pressure to the core-mantle boundary reaches the liquidus together.

The numbers below hold for the tables of 1 to 10 Earth-mass planets, sampled in steps of 0.25 Earth masses, with `mushy_zone_factor = 0.8`, the default `lookup_nP = 1350`, and the pressure range $P_\mathrm{max} = \min(10\,\mathrm{TPa},\ 150\,\mathrm{GPa} \cdot M/M_\oplus + 200\,\mathrm{GPa})$. The peak of the smoothed liquidus moves with the table range: it lies between about 110 and 255 GPa and can jump between neighbouring masses. The smoothed solidus can peak lower, between about 78 and 175 GPa, so its plateau can start at a much lower pressure than that of the liquidus. The liquidus plateau lies above the smoothed curve by 64 to 152 J/kg/K at 350 GPa and by 145 to 991 J/kg/K at the table top; the solidus offset reaches 13 to 104 J/kg/K. In temperature, the written liquidus entropy is reached on the liquid table 62 to 149 K above the smoothed curve at 350 GPa (88 to 144 K above the PALEOS liquidus), and at the table top, where the smoothed curve meets the PALEOS liquidus, 142 K above it for 1 Earth mass and 1645 K above it for 10 Earth masses. Material on the liquidus at the table top therefore has a lever-rule melt fraction of 0.98 (1 Earth mass), 0.91 (5) and 0.85 (10), not 1. With both boundaries raised, the lever-rule melt fraction is never higher than the smoothed curves give. Above both peaks the entropy of fusion is $S_\mathrm{liq,peak} - S_\mathrm{sol,peak}$, larger than the smoothed gap by up to 2 % (1 Earth mass) and 15 % (10 Earth masses) at the table top; where the running maximum raises the solidus more than the liquidus, the gap is smaller than the smoothed gap by at most 0.3 %. The smoothing itself departs from the table entropy by up to about 240 J/kg/K on the liquidus and 190 J/kg/K on the solidus, so against the table the melt fraction can be higher or lower.
...
With `use_jax = true` and `temperature_arrays` supplied, Zalmoxis takes T from the arrays in every part of the solve: the JAX structure integration, its numpy fallback, the density update at the nodes inside `_solve`, and the output temperature profile. Zalmoxis then ignores `temperature_function`, and the wrapper withholds it on JAX-viable EOS layouts. On the setup of `tools/benchmarks/bench_coupled_tempfunc.py` (`tests/data/bench_performance.toml`, 2-phase PALEOS mantle, CHILI T profile, `outer_solver = "picard"`, `use_anderson = false`), one Zalmoxis call makes 1619 `solve_structure` calls with the nodal T from the arrays. With the nodal T from the internal `temperature_mode` profile instead, the same call makes 4916.
