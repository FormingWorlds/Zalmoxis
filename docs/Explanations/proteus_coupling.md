...
Cells without a PALEOS state, such as the melt table below the liquidus or the solid table extended up to the melt entropy range, are filled from the nearest valid cell in the same pressure column, because SPIDER cannot read NaN. `valid_mask_solid.dat` and `valid_mask_melt.dat`, written on the same grid and in the same format, mark each cell 1 (PALEOS state) or 0 (filled). The solidus and liquidus entropies are held at their running maximum in pressure, so above their peak the written curves are flat.

**Liquidus entropy plateau.** For MgSiO3 the liquid-table entropy along the liquidus peaks and then falls with pressure while the liquidus temperature keeps rising, since $\mathrm{d}S_\mathrm{liq}/\mathrm{d}P = -\alpha/\rho + (c_p/T)\,\mathrm{d}T_\mathrm{liq}/\mathrm{d}P$. With the falling curve, the first solid in a cooling molten mantle forms in a thin layer at the peak above still molten material, and the time step of coupled runs of 3 and 5 Earth-mass planets (SPIDER at both masses, Aragog at 3) collapses at that onset. The plateau is therefore a solver-stability choice, not a property of the tables: the whole range from the peak down to the core-mantle boundary reaches the liquidus together. The peak pressure and the plateau offsets depend on the pressure range of the table, because the smoothing anchors follow that range. For the tables of 1 to 10 Earth-mass planets ($P_\mathrm{max}$ = 350 to 1700 GPa) the peak lies between about 120 and 225 GPa; the plateau lies 100 to 150 J/kg/K above the table liquidus entropy at 350 GPa and 145 to 990 J/kg/K at the table top, and the solidus offset reaches 13 to 104 J/kg/K. Because both boundaries are raised, the lever-rule melt fraction above the peak is never higher than the table curves give, and a cell with an entropy between the table liquidus and the plateau counts as partly molten instead of molten.
