...
2. `PALEOS-2phase:<solid>` (for example `PALEOS-2phase:MgSiO3`): separate solid + liquid tables. The liquidus is the PALEOS liquidus and the solidus is `mushy_zone_factor * liquidus`, the same curve pair that the structure solve receives for the two-phase density and the mushy-zone adiabatic gradient. The gap between the solid-table top and the liquid-table bottom sets the latent heat.

Cells without a PALEOS state, such as the melt table below the liquidus or the solid table extended up to the melt entropy range, are filled from the nearest valid cell in the same pressure column, because SPIDER cannot read NaN. `valid_mask_solid.dat` and `valid_mask_melt.dat`, written on the same grid and in the same format, mark each cell 1 (PALEOS state for every property) or 0 (a value was filled, or, for a missing adiabatic gradient, written as 0). The solidus and liquidus entropies are held at their running maximum in pressure, so above their peak the written curves are flat.

**Liquidus entropy plateau.** The written solidus and liquidus entropies come from a PCHIP fit through 20 anchors spread over the pressure range of the table, and each curve is then held at its running maximum in pressure, so at pressures above its peak it is flat. For MgSiO3 the liquid-table entropy along the liquidus peaks near 164 GPa and then falls while the liquidus temperature keeps rising, since $\mathrm{d}S_\mathrm{liq}/\mathrm{d}P = -\alpha/\rho + (c_p/T)\,\mathrm{d}T_\mathrm{liq}/\mathrm{d}P$. With the falling curve, the first solid in a cooling molten mantle forms in a thin layer at the peak above still molten material, and the time step of coupled SPIDER and Aragog runs of 3 and 5 Earth-mass planets collapses at that onset. The plateau is therefore a solver-stability choice, not a property of the tables: the whole range from the peak pressure to the core-mantle boundary reaches the liquidus together. The peak of the smoothed curve moves with the table range: for the tables of 1 to 10 Earth-mass planets (`mushy_zone_factor = 0.8`) it lies between about 110 and 255 GPa and can jump between neighbouring masses, and the smoothed solidus can peak lower, between about 78 and 175 GPa, so its plateau can start at a much lower pressure than that of the liquidus. The liquidus plateau lies above the smoothed curve by 64 to 152 J/kg/K at 350 GPa and by 145 to 991 J/kg/K at the table top; the solidus offset reaches 13 to 104 J/kg/K. With both boundaries raised, the lever-rule melt fraction is never higher than the smoothed curves give. The smoothing itself departs from the table entropy by up to about 240 J/kg/K on the liquidus and 190 J/kg/K on the solidus, so against the table the melt fraction can be higher or lower.
