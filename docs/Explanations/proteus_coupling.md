...
4. Call `zalmoxis_solver(config, outdir, hf_row, num_spider_nodes=..., temperature_function=temperature_function, temperature_arrays=(r_arr, T_arr))`. With `use_jax = true`, Zalmoxis takes T from the arrays everywhere and does not use the callable; with `use_jax = false` it uses the callable (see [Outer solver and JAX path](#outer-solver-and-jax-path)).
...
With `use_jax = true` and `temperature_arrays` supplied, Zalmoxis takes T from the arrays in every part of the solve: the JAX structure integration, its numpy fallback, the density update at the nodes inside `_solve`, and the output temperature profile. The callable is then not used, and the wrapper withholds it on JAX-viable EOS layouts. Taking the node T from the arrays does not slow the Picard outer solver: on the setup of `tools/benchmarks/bench_coupled_tempfunc.py` (`tests/data/bench_performance.toml`, 2-phase PALEOS mantle, CHILI T profile) it converges in 1619 structure solves, against 4916 with the node T from the internal linear profile.
