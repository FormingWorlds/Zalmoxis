...
**Liquidus entropy plateau.** For MgSiO3 the liquid-table entropy along the liquidus peaks between about 125 and 175 GPa (the smoothing anchors follow the pressure range of the table) and falls at higher pressure while the liquidus temperature keeps rising, since $\mathrm{d}S_\mathrm{liq}/\mathrm{d}P = -\alpha/\rho + (c_p/T)\,\mathrm{d}T_\mathrm{liq}/\mathrm{d}P$. With the falling curve, the first solid in a cooling molten mantle forms in a thin layer at the peak above still molten material, and the time step of coupled runs of 3 and 5 Earth-mass planets (SPIDER at both masses, Aragog at 3) collapses at that onset. The plateau is therefore a solver-stability choice, not a property of the tables: the whole range from the peak down to the core-mantle boundary reaches the liquidus together. The plateau lies above the table liquidus entropy by about 150 J/kg/K at 350 GPa and about 640 J/kg/K at 950 GPa; the solidus offset is at most about 60 J/kg/K. Because both boundaries are raised, the lever-rule melt fraction above the peak is never higher than the table curves give, and a cell with an entropy between the table liquidus and the plateau counts as partly molten instead of molten.
