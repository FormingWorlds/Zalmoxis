...
Three shields.io endpoint-badge JSON files are published to the dedicated
`badges` branch:
...
The `Refresh test count badges` GitHub Actions workflow regenerates the
counts on every push to `main` whose paths touch `tests/`, `src/`,
`pyproject.toml`, the workflow YAML, or `tools/generate_test_badges.py`,
and publishes the JSON to the `badges` branch. The `badges` branch holds
only these three files and is kept separate from `main` so the badge
refresh never needs a direct push to the protected default branch.
Shields.io fetches the JSON from the raw GitHub URL of that branch and
renders the badge live at the top of this page and on the PROTEUS
