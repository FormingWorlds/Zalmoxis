...
The repository publishes shields.io endpoint-badge JSON files at `.github/badges/tests-*.json` on every push to `main`. These power the test-count badges shown in the README and on the PROTEUS framework website at [https://proteus-framework.org/testing](https://proteus-framework.org/testing). The JSON files are regenerated by the `Refresh test count badges` GitHub Actions workflow; the regeneration script is `tools/generate_test_badges.py`.
