...
def run_full_paleos(mass_earth, nabla_func, f_a=0.04):
    """Tier 3: PALEOS structure + PALEOS nabla_ad + PALEOS C_p."""
    ...
    # Get PALEOS C_p at CMB conditions
    cmb_idx = int(np.argmin(np.abs(
        np.array(res['mass_enclosed']) - CMF * res['mass_enclosed'][-1])))
    P_cmb = float(res['pressure'][cmb_idx])
    # Estimate T_cmb for C_p query (use ~4000 K as starting guess)
    T_est = 4000.0 + (mass_earth - 1.0) * 2000.0
    C_fe = get_paleos_cp_at_conditions(P_cmb, T_est, 'iron')
    C_sil = get_paleos_cp_at_conditions(P_cmb, T_est, 'MgSiO3')

    th = initial_thermal_state(res, CMF, T_radiative_eq=T_EQ,
                               f_accretion=f_a, f_differentiation=F_D,
                               C_iron=C_fe, C_silicate=C_sil,
                               nabla_ad_func=nabla_func)
    return {'res': res, 'th': th, 'C_fe': C_fe, 'C_sil': C_sil,
            'label': 'PALEOS (full)'}
...
def main():
    print('Building PALEOS nabla_ad...')
    ...
    for m in MASSES:
        ...
        r3 = run_full_paleos(m, nabla_func)
        ...
        if r3:
            ...
            C_info = f'C_Fe={r3["C_fe"]:.0f}, C_sil={r3["C_sil"]:.0f}'
    ...
    for f_a in F_A_VALUES:
        fa_results[f_a] = []
        for m in MASSES:
            r = run_full_paleos(m, nabla_func, f_a=f_a)
            if r:
    ...
    print(f'{"M/ME":>5}  {"T1_CMB":>7}  {"T1_surf":>8}  {"T2_CMB":>7}  {"T2_surf":>8}  '
          f'{"T3_CMB":>7}  {"T3_surf":>8}  {"C_fe":>5}  {"C_sil":>5}  {"core3":>7}')
    print('-' * 100)
    for i in range(min(len(tier1), len(tier2), len(tier3))):
        ...
        print(f'{t1["mass"]:5.1f}  {t1["T_cmb"]:7.0f}  {t1["T_surface"]:8.0f}  '
              ...
              f'{t3.get("C_avg", 0):5.0f}  {t3.get("C_avg", 0):5.0f}  '
              f'{t3["core_state"]:>7}')
    print('=' * 100)
