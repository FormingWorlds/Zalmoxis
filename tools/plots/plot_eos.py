...
def plot_eos_Seager2007(data_files, data_folder):
    ...
    ax.set_yscale('log')
    ax.legend()
    fig.savefig(os.path.join(get_zalmoxis_root(), 'output', 'planet_eos_Seager2007.pdf'))
    # plt.show()
    plt.close(fig)
...
def plot_eos_WolfBower2018(data_file, data_folder, melting_data_folder, melting_data_files):
    ...
    if data_file == 'density_melt.dat':
        ...
        label = 'MgSiO3 solid'
    fig.savefig(
        os.path.join(get_zalmoxis_root(), 'output', f'planet_eos_WolfBower2018_{label}.pdf')
    )
    # plt.show()
    plt.close(fig)
