...
By convention the trajectory runs from the planet centre (high P, high
T) to the surface (low P, low T). Pressure is on the y-axis in GPa and
is log-scaled by default because interior pressures span several
decades.
...
The script is also importable::
    ...
    plot_grid_pt("output/grid_mass_radius")
"""
...
from zalmoxis.config import _NEEDS_MELTING_CURVES as _EOS_USES_EXTERNAL_CURVES

# ---------------------------------------------------------------------------
# Constants (kept in sync with tools/plots/plot_grid_profiles.py)
# ---------------------------------------------------------------------------
_FIXED_COLUMNS = {
    'label',
    'R_earth',
...
def _mantle_uses_external_curves(mantle_eos):
    ...
    The mantle string may be a single component (``"PALEOS:MgSiO3"``) or
    a multi-component mixture with mass fractions
    (``"PALEOS:MgSiO3:0.9+Chabrier:H:0.1"``). Any component in the
    ``_EOS_USES_EXTERNAL_CURVES`` set triggers the overlay.
    """
    if not mantle_eos:
        return False
    # Mixture separator in Zalmoxis is '+'; strip any mass-fraction
    # suffix (``:<float>``) before checking membership.
    components = []
...
def plot_grid_pt(
    ...
    colour_by : str, optional
        Sweep-parameter column to colour trajectories by. Defaults to
        the first numeric sweep parameter.
    ...
    # overlaid melting curves span the same domain and do not stretch
    # the plot when an analytic curve extrapolates to unphysical T at
    # very high P (e.g. Monteux+16 is documented valid only to ~500
    # GPa).
    p_min = float('inf')
    p_max = 0.0
    t_max_data = 0.0
    ...
    # Overlay mantle solidus / liquidus curves, but only if they
    # actually correspond to the curves the solver used for this grid.
    curve_handles = []
    overlay_note = None
