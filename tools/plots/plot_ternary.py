...
def run_zalmoxis_for_ternary(args):
    ...
    with open(custom_log_file, 'a') as log:
        log.write(
            f'{core_frac:.4f}\t{mantle_frac:.4f}\t{water_frac:.4f}\t{planet_radius:.4e}\t{total_time:.4e}\n'
        )
    return core_frac, mantle_frac, water_frac, converged
...
def run_ternary_grid_for_mass(id_mass=None, step=0.05):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        results = []
        for future in tqdm(
            as_completed(futures), total=len(futures), desc=f'Processing mass {id_mass}'
        ):
...
def plot_ternary(data, id_mass):
    ...
    # plt.show()
    plt.savefig(
        os.path.join(get_zalmoxis_root(), 'output', f'ternary_diagram{id_mass}.png'), dpi=300
    )
...
def plot_ternary_time(data, id_mass):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(get_zalmoxis_root(), 'output', f'ternary_diagram_time{id_mass}.png'),
        dpi=300,
    )
