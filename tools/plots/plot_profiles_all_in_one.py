...
def plot_profiles_all_in_one(target_mass_array, choice):
    ...
    Data files:
        - Planet profile files: 'planet_profile{id_mass}.txt' in the output directory.
    ...
    Output:
        The generated plot is saved as 'all_profiles_with_colorbar_vs_{choice}.pdf' in the output directory.
    ...
    for id_mass in target_mass_array:
        ...
        file_path = os.path.join(get_zalmoxis_root(), 'output', f'planet_profile{id_mass}.txt')
    ...
    cmap = cm.plasma
    norm = Normalize(
        vmin=1, vmax=np.max(target_mass_array)
    )  # Normalize the id_mass range to map to colors
    ...
    if choice == 'Wagner':
        axs[0, 0].scatter(
            ...
            color='black',
            s=1,
            label='Earth-like super-Earths (Wagner et al. 2012)',
        )
    elif choice == 'Boujibar':
        axs[0, 0].scatter(
            boujibar_radii_for_densities,
            ...
            color='black',
            s=1,
            label='Earth-like super-Earths (Boujibar et al. 2020)',
        )
    elif choice == 'default':
        pass
    elif choice == 'SeagerEarth':
        axs[0, 0].scatter(
            ...
            color='black',
            s=5,
            label='Earth-like super-Earths (Seager et al. 2007)',
            zorder=10,
        )
    elif choice == 'Seagerwater':
        axs[0, 0].scatter(
            ...
            color='black',
            s=5,
            label='Water planets with iron core and silicate mantles (Seager et al. 2007)',
            zorder=10,
        )
    elif choice == 'custom':
        pass
    ...
    plt.savefig(
        os.path.join(
            get_zalmoxis_root(), 'output', f'all_profiles_with_colorbar_vs_{choice}.pdf'
        )
    )
    # plt.show()
    plt.close(fig)
