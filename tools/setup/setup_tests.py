...
def run_zalmoxis_TdepEOS(id_mass):
    ...
    custom_log_file = os.path.join(
        get_zalmoxis_root(), 'output', 'composition_TdepEOS_mass_log.txt'
    )
...
def run_zalmoxis_RTPress100TPa(id_mass):
    """Run Zalmoxis with RTPress100TPa:MgSiO3 mantle EOS for a given mass.
    ...
    # Check if model converged before proceeding
    if not converged:
        print(f'Model did not converge for mass {id_mass} Earth masses (RTPress100TPa).')
        return [(id_mass, False, model_results)]
    ...
    # Log the mass and radius only if converged
    custom_log_file = os.path.join(
        get_zalmoxis_root(), 'output', 'composition_RTPress100TPa_mass_log.txt'
    )
    with open(custom_log_file, 'a') as log:
        log.write(f'{id_mass:.4f}\t{planet_radius:.4e}\t{total_time:.4e}\n')
    return [(id_mass, converged, model_results)]
...
def run_zalmoxis_PALEOS(id_mass, temperature_mode='linear'):
    ...
    if not converged:
        print(f'Model did not converge for mass {id_mass} Earth masses (PALEOS).')
        return [(id_mass, False, model_results)]
    ...
    custom_log_file = os.path.join(
        get_zalmoxis_root(), 'output', 'composition_PALEOS_mass_log.txt'
    )
    with open(custom_log_file, 'a') as log:
        log.write(f'{id_mass:.4f}\t{planet_radius:.4e}\t{total_time:.4e}\n')
    return [(id_mass, converged, model_results)]
