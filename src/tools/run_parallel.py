...
def run_zalmoxis_in_parallel(choice):
    ...
    if choice == 'Wagner':
        ...
        logger.info(
            'No choice selected for the comparison, defaulting to 1 to 10 Earth masses simulation.'
        )
        target_mass_array = range(1, 11)
        ...
        raise ValueError(
            "Invalid choice. Please select 'Wagner', 'Boujibar', 'default', 'SeagerEarth', 'Seagerwater', or 'custom'."
        )
...
if __name__ == '__main__':
    ...
    logger.info(
        f"Total time for running Zalmoxis in parallel with choice '{choice}' is: {total_time:.2f} seconds"
    )
