...
def plot_mass_radius_relationship(target_mass_array):
    ...
    zeng_masses_Earth = []
    zeng_radii_Earth = []
    with open(os.path.join(ZALMOXIS_ROOT, "data", "mass_radius_curves", "massradiusEarthlikeRockyZeng.txt"), 'r') as zeng_file:
    ...
    zeng_masses_water = []
    zeng_radii_water = []
    with open(os.path.join(ZALMOXIS_ROOT, "data", "mass_radius_curves", "massradiuswaterZeng.txt"), 'r') as zeng_file:
        next(zeng_file)  # Skip the header line
        for line in zeng_file:
            mass, radius = map(float, line.split())
